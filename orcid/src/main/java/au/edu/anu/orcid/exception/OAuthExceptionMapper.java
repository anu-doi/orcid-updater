...
/**
 * <p>OAuthExceptionMapper</p>
 *
 * <p>The Australian National University</p>
 *
 * <p>ExceptionMapper class for the OAuthException class.  This either displays a web page with the
...
public class OAuthExceptionMapper implements ExceptionMapper<OAuthException> {
	...
	public Response toResponse(OAuthException exception) {
		...
		if (acceptableMediaTypes.contains(MediaType.TEXT_HTML_TYPE)) {
			...
			resp = Response.status(Status.INTERNAL_SERVER_ERROR).entity(new Viewable("/jsp/error.jsp", model)).build();
		}
		else {
			resp = Response.status(Status.INTERNAL_SERVER_ERROR).entity(exception.getMessage()).build();
		}
